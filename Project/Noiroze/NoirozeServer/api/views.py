...
class UserLoginView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():                            # 유저 시리얼라이저 확인 후
            ...
            # print(user)
            response.set_cookie('auth-token', access_token)              # 쿠키에 auth-token 이라는 이름으로 토큰 키값 저장
            return response
...
class CommunityBoardViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if category == "전체보기": 
            self.queryset = CommunityBoard.objects.all()
...
class ComplainBoardViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [AllowAny]                                  # 토큰 검증
    serializer_class = ComplainBoardSerializer                              # 민원접수 게시판 데이터에 대한 직렬화 처리
    pagination_class = SetPagination

    def get_queryset(self):
        user = self.request.user
        if user.is_anonymous:                                               # user가 anonymous인 경우
            return ComplainBoard.objects.all().order_by('-id')
        if user.is_superuser:                                               # user가 admin인 경우
            '''
            complain_boards = ComplainBoard.objects.all()
            for board in complain_boards:
                print(board.created_date)
            '''
            return ComplainBoard.objects.all().order_by('-id')
        else:
            # print(ComplainBoard.created_date)
