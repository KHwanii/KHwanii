...
import com.example.myapp.ui.community.service.CreateCommunityBoardRequest
...
class BoardCreateFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        txt_select_category.setOnClickListener {
            ...
            val listItems = arrayListOf("정보공유", "소통해요", "칭찬해요", "나눔해요") // 카테고리 종류
        ...
        btn_complete.setOnClickListener{
            ...
            if (category.isNotEmpty() && title.isNotEmpty() && content.isNotEmpty()) {                                  // 각 필드가 비어있지 않은지 확인
                ...
                val request = CreateCommunityBoardRequest(category, title, content, authorId, created_date)             // author와 created_date는 서버에서 처리
                val boardCreateCall = CommunityBoardSetup.service.createCommunityBoard(request)                         // 서버에 POST 요청
                boardCreateCall.enqueue(object : Callback<ResponseBody> {
                    override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                        if (response.isSuccessful) {
                            showToast(context, "게시글이 작성되었습니다.")
                            findNavController().navigateUp()                                            // 요청이 성공하면 이전 화면으로 돌아감
                        }
                        else {
                            // val message = response.errorBody()?.string()                             // 서버 에러 메시지 토스트메세지로 보여줌
                            Log.d("BoardCreateError", "Error")
                        }
                    }

                    override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                        Log.e("BoardCreate Error: ", t.message.toString())                         // 요청이 실패하면 로그에 에러를 출력
                    }
                })
            }
            else {
                Toast.makeText(context, "모든 필드를 채워주세요.", Toast.LENGTH_SHORT).show()         // 필드 중 하나라도 비어있으면 알림
            }
        }
    }
